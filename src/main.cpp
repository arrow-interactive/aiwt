    ...
    for(std::string line : lines)
    {
        ...
        for(size_t i = 0; i < line.length(); i++)
        {
            ...
            if(ch == '#')
            {
                ...
                if(code_block)
                {
                    std::string tmp = line.substr(substr_start);

                    replace_all(tmp, "&", "&amp;");
                    replace_all(tmp, "<", "&lt;");
                    replace_all(tmp, ">", "&gt;");

                    out += "<h" + std::to_string(heading_level);
                    for(auto attr : attrs)
                        out += ' ' + attr.first + '=' + attr.second;
                    out += '>' + tmp;
                    if(out.substr(out.size() - 2).compare("  ") == 0)
                        out.replace(out.size() - 2, 2, "<br>");
                    out += "</h>\n";
